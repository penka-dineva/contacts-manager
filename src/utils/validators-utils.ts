...
export function validateIban(): ValidatorFn {
  const mod97 = (val: string) => {
    let checksum: string | number = val.slice(0, 2), fragment;
    for (var offset = 2; offset < val.length; offset += 7) {
 ...
 const validate = (control: AbstractControl) => {
  ...
  if (control.value) {
    ...
    return mod97(digits.toString()) === 1 ? null : { ibanValid: false };
  }
